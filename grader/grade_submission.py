...
A case passes when |output - expected| <= 0.001 + 1e-4 * |expected|.
...
from helpers import ABS_TOL, INPUT_KEYS, REL_TOL, load_golden, load_module, load_oracle, tolerance  # noqa: E402
...
def _predictions_from_module(path, cases):
    ...
    info = {}
    if hasattr(mod, "fit"):
        budget = min(int(getattr(mod, "PROBE_BUDGET", DEFAULT_BUDGET)), DEFAULT_BUDGET)
...
def main(argv):
    ...
    report.update(info)
    if os.environ.get("GRADER_VERBOSE") != "1":
